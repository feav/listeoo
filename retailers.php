	...
	?>
		...
		<?php }else{ ?>
			...
			<?php while ($row = mysqli_fetch_array($result)) { $cc++; ?>
				...
				-->
						<a class="go2store" href="<?php echo SITE_URL; ?>go2store.php?id=<?php echo $row['retailer_id']; ?>" target="_blank"><?php echo DisplayCashback($row['cashback']); ?> Cashback!</a>
					</div>
				</div>
				<?php } ?>
